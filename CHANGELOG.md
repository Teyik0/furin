...
### Added
- **Native PostgreSQL sync notifications** — `postgresSyncNotifier()` uses Bun `SQL.listen()` / `SQL.notify()` with namespace-isolated channels, listener teardown, and durable cursor recovery after reconnects.
...
### Fixed
- **Single sync startup catch-up** — browser sync opens SSE before issuing one initial `/changes?after=0` read, removing the redundant pre-stream request while preserving paginated journal recovery.
...
- **Performance regression budgets** — pull requests compare the docs client bundle and embedded weather binary against their target branch, blocking regressions beyond explicit size thresholds while retaining startup, load, CPU, and heap diagnostics as artifacts.
- **Static preview and redirects** — `furin preview` serves static exports with Bun's native `_client` directory route, while static loader redirects now emit portable HTML redirect pages instead of being omitted.
