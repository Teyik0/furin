...
- **Bun 1.4 build primitives** — production clients use Bun's native Rust React Compiler and code splitting by default, compiled servers use split ESM bytecode, generated build entries are provided in memory through `Bun.build({ files })`, `optimizeImports` delegates explicit barrel optimization to Bun, and `--analyze` emits complete client metafiles for entrypoint, lazy-chunk, and source-contribution analysis.
- **Static preview and redirects** — `furin preview` serves static exports with Bun's native directory routes, including base paths, ETags, conditional requests, byte ranges, nested pages, custom 404s, and trailing-slash normalization; static loader redirects now emit portable HTML redirect pages.
- **Centralized development diagnostics** — a process-stable `DevGraph` owns route snapshots, module revisions, dependency chains, loader/import caches, HMR events, and metrics. A source-mapped browser overlay reports transform, import, loader, render, and hydration failures with route context, causes, import chains, retry, and automatic recovery.
- **Browser HMR conformance coverage** — real-browser tests verify state-preserving compatible edits, remounts after hook-signature changes (including `memo`/`forwardRef`), server reconnect recovery, atomic component/data updates, and an opt-in 2,000-edit soak run.
- **Live route-contract refresh coverage** — persistent TypeScript and browser tests verify that params, query, loader, cache-tag, rendering-mode, and schema edits propagate during development without restarting the server.
...
- **Automated performance reporting** — deterministic client-bundle and compiled-binary budgets block regressions, while a credential-gated Vercel workflow publishes live Furin, Next.js, and TanStack Start latency comparisons on pull requests and weekly runs.
- **Dynamic weather example** — the weather application now demonstrates typed `/weather/:city` routes, ISR `staticParams()`, typed links, an SSR search redirect, mounted `basePath` support, deterministic UTC weekday formatting, and shared page-loading logic with route-level integration coverage.

### Changed
- **Filesystem-driven development graph** — route topology polling is replaced by filesystem watchers, content signatures, debounced reconciliation, monotonic revisions, and retry after failed rebuilds; route-module and dependency invalidation now flows through one graph.
- **Unified browser event transport** — sync, diagnostics, DevTools, and HMR lifecycle events share one versioned `/_furin/events` WebSocket with bounded buffering, heartbeats, replay cursors, and jittered reconnects.
- **Self-contained Bun binaries use native asset directories** — `--compile embed` now embeds client and public directories through Bun 1.4's `compile.assets` support and serves them through the same production static path as disk builds, removing Furin's generated per-file asset map and handlers.
- **Compact navigation responses and earlier streaming** — plain SSG/ISR route data uses a compact JSON envelope when lossless CrossJSON/route-frame features are unnecessary, while deferred SSR flushes the public shell earlier.
- **Zero-copy synchronous render context** — the loader result is now the canonical render context passed through server and browser adapters to pages and layouts. Furin only allocates a merged object when deferred data is present and an isolated shallow copy when `head()` executes, removing the unconditional render-context mapping without exposing hydration state to metadata mutations.
- **Bun server entry convention** — generated applications export their numeric `port`, guard `.listen()` with `import.meta.main`, and default-export the Elysia application so the same entry works for local Bun execution, compiled output, and Vercel Functions.
...
- **Vercel cache and stream correctness** — successful cacheable navigation payloads share the document's invalidation tags, query-dependent routes remain on demand, external prerendering bypasses process-local ISR state, and request-private PPR continuation state is authenticated and kept out of browser HTML.
- **Render context on failures** — SPA loader errors retain validated `params`, `path`, and `query`; SSR fallbacks receive the same route context; and root 404 rendering parses the root query schema with coercion, defaults, and repeated search parameters before rendering the layout.
- **Isolated metadata execution** — mutations performed inside `head()` can no longer alter page props or serialized hydration data, while routes without `head()` keep the zero-copy path.
- **Compiled application ports** — the generated Bun boot entry honors an exported numeric `port` whenever `PORT` is unset, while an explicit runtime `PORT` still takes precedence. Compiled workspace applications therefore retain their distinct local production defaults.
- **Stable HMR topology tests** — route-removal coverage waits for the dev route graph to converge to a 404 before the next schema test starts, eliminating the full-suite race without weakening the assertion.
