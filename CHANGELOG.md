...
### Added
- **Bun 1.4 build primitives** — production clients use Bun's native Rust React Compiler and code splitting by default, compiled servers use split ESM bytecode, generated build entries are provided in memory through `Bun.build({ files })`, and `--analyze` emits complete client metafiles for entrypoint, lazy-chunk, and source-contribution analysis.
