...
function importedLoaderSource(version: string): string {
  return [
    ...
    "export function FeaturePage({ message }: { message: string }) {",
    `  return <main data-version="${version}"><output data-testid="loader">{message}</output></main>;`,
    "}",
  ].join("\n");
}
...
function loaderPageSource(version: string, loaderThrows: boolean): string {
  ...
  return [
    ...
    'import { route as rootRoute } from "./root";',
    "",
    "function LoaderPage({ message }: { message: string }) {",
    "  const [count, setCount] = useState(0);",
    "  return (",
    `    <main data-version="${version}">`,
    '      <output data-testid="loader">{message}</output>',
    '      <output data-testid="count">{count}</output>',
    '      <button data-testid="increment" onClick={() => setCount((value) => value + 1)}>',
    "        Increment",
    "      </button>",
...
function deferredPageSource(version: string): string {
  return [
    ...
    'import { route as rootRoute } from "./root";',
    "",
    "function DeferredPage({ message }: { message: Promise<string> }) {",
    "  const [count, setCount] = useState(0);",
    "  return (",
    `    <main data-version="${version}">`,
    '      <output data-testid="count">{count}</output>',
    ...
    '        <Await resolve={message}>{(resolved) => <output data-testid="deferred">{resolved}</output>}</Await>',
    "      </Suspense>",
    "    </main>",
    "  );",
    "}",
