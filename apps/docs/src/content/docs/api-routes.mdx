...
The sync stream endpoint is internal and opinionated: `/_furin/sync`. SSE announces the current cursor when the stream opens and every later cursor change; the client recovers ordered invalidations through `/_furin/sync/changes`. Passing the explicit runtime to `furin({ sync })` injects the client configuration automatically, so you do not need a React provider or application wrapper.
...
const postgres = { namespace: "task-manager", sql }
const sync = {
  adapter: postgresSyncAdapter(postgres),
...
The PostgreSQL notifier uses Bun's native `SQL.listen()` and `SQL.notify()` support. Its channel is derived from the namespace, and the adapter queues `NOTIFY` in the same transaction as durable completion so it is delivered only after commit. The listener reconnect callback reads the durable cursor to recover notifications missed while disconnected. No periodic cursor polling runs while the listener is healthy; a failed initial subscription restores the safety poll.

Distributed production adapters require an explicit notifier. If `LISTEN/NOTIFY` is unavailable through an intermediary such as an incompatible connection pooler, opt into the polling fallback:
...
An independent `redisSyncNotifier()` remains available when Redis is already part of the deployment.

| Deployment | Choice |
|---|---|
| Local development or one disposable process | SQLite `:memory:` |
| Durable deployment on one host | SQLite adapter |
| Multi-replica with an application PostgreSQL database | PostgreSQL adapter + PostgreSQL notifier (recommended) |
| Redis is already the durable sync source | Redis adapter |
| PostgreSQL durability with an existing Redis bus | PostgreSQL adapter + Redis notifier |

The notifier only sends a small cursor and is best-effort. The durable adapter owns response replay and ordered catch-up, so a missed PostgreSQL, Redis, or SSE notification does not lose changes. Completion atomically stores the replay response and its invalidations. This is not a claim of exactly-once execution for arbitrary domain writes: Furin cannot put an external API call or an application database transaction inside the adapter transaction.
