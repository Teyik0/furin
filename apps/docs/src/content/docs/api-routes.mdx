...
The PostgreSQL notifier uses Bun's native `SQL.listen()` and `SQL.notify()` support. Its channel is derived from the namespace, and its reconnect callback reads the durable cursor to recover notifications missed while disconnected. No periodic cursor polling runs in this setup.
