...
Known gaps in the Bun-backed development pipeline remain executable with
`test.failing`, so CI fails if their
behavior changes without the expectation being reviewed. They currently cover:
...
These expected failures mean the suite is a parity gate, not a claim that parity
has already been reached. A global reload on every update would hide the gaps but
