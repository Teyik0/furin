...
| **RSC** | ✅ | ⚠️ Experimental | ✅ Composite RSC |
| **PPR** | ✅ Cache Components | ❌ | ✅ (`requestLoader` + Suspense) |
| **Binary compile** | ❌ | ❌ | ✅ |
...
| **RSC (React Server Components)** | ✅ | ⚠️ Experimental | ✅ Composite RSC |
| **PPR (Partial Prerendering)** | ✅ Cache Components | ❌ | ✅ (`requestLoader` + Suspense) |
...
>
> ⚠️ **Furin limitation** — Furin's composite RSC model is explicit: server-owned markup is returned by a loader and client interactivity enters through typed slots. Unlike Next.js App Router, components are not server components by default and Furin does not provide Server Actions.
>
> 💡 **Furin note** — `defer()` is supported on SSR routes. For SSG and ISR personalization, use `requestLoader` below an explicit Suspense boundary.
...
| **Typed params / query** | ⚠️ Zod manual | ✅ (`validateSearch`, `params`) | ✅ (`defineRoute().config({ params: t.Object(...) })`) |
...
## Mutations & Multi-replica Sync

| Dimension | Next.js | TanStack Start | Furin |
|---|---|---|---|
| **Mutation contract** | Server Actions or route handlers | Server Functions or server routes | Elysia HTTP routes + Eden Treaty |
| **On-demand cache invalidation** | `revalidatePath`, `revalidateTag`, `updateTag` | Coordinate router/query cache in application code | `revalidatePath`, `revalidateTag`, path/tag invalidation rules |
| **Durable idempotency journal** | Application responsibility | Application responsibility | SQLite, PostgreSQL, or Redis adapter |
| **Missed-event catch-up** | Application responsibility | Application/query-library responsibility | Ordered cursor journal + SSE wake-up |
| **Optional backend isolation** | Deployment/runtime dependent | Environment code is build-time separated | Independent side-effect-free package subpaths |

Next.js has the broadest integrated cache revalidation API. TanStack Start emphasizes typed same-origin Server Functions and deliberate client-cache integration. Furin keeps standard HTTP routes as the public mutation contract and adds an optional durable replay/change journal around them. The recommended production topology uses PostgreSQL for both durability and native `LISTEN/NOTIFY` wake-ups, with Redis remaining an optional independent notifier.

Furin adopts the immediately useful local-first behavior described in the [Linear performance analysis](https://performance.dev/how-is-linear-so-fast-a-technical-breakdown): `useSync()` applies optimistic state synchronously, SSE carries only a cursor, and catch-up is coalesced. It deliberately does not pretend that a server journal is a browser database. IndexedDB outbox, offline mutation replay, and conflict resolution remain a separate opt-in project.

Sources: [Next.js revalidation](https://nextjs.org/docs/app/getting-started/revalidating), [TanStack Start Server Functions](https://tanstack.com/start/latest/docs/framework/react/guide/server-functions), and [TanStack environment tree-shaking](https://tanstack.com/start/latest/docs/framework/react/guide/environment-functions).
...
| Dimension | Next.js | TanStack Start | Furin |
|---|---|---|---|
| **Route loaders** | ✅ (`async function Page()`) | ✅ (`Route.loader`) | ✅ (`defineRoute().loader()`) |
| **Layout loaders** | ✅ (`async function Layout()`) | ✅ (`Route.loader`) | ✅ (`_route.tsx` loader) |
| **Typed loader output** | ⚠️ (TypeScript inference, no runtime validation) | ✅ (Zod/Valibot via `validateSearch`) | ✅ (TypeBox via `params`/`query` on `defineRoute()`) |
...
> 💡 **Furin advantage** — Loaders are standard async functions with TypeBox schemas for params/query. No magic conventions: what you see in `defineRoute()` is what gets validated at runtime.
>
...
> 💡 **Furin advantage** — `defineRoute()` is the single source of truth for params, query, and loader types. The generated `furin-env.d.ts` gives autocomplete on `<Link>` without any runtime overhead.
>
