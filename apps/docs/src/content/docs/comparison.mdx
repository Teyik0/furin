...
> - **TanStack Start** = v1 stable (server functions and PPR are experimental and excluded).
...
| **HTTP server** | Custom (Node) | Vite (dev) / Nitro (prod) | **Elysia** |
| **Bundler** | Turbopack / webpack | Vite | **Bun.build** |
| **Process model** | Separate dev server + client bundler | Single Vite process (dev) / Nitro (prod) | **Single process** |
| **SSG** | ✅ | ✅ | ✅ |
| **ISR** | ✅ | ❌ | ✅ |
| **SSR** | ✅ | ✅ | ✅ |
| **Streaming** | ✅ | ✅ | ✅ |
...
| **HTTP framework** | Custom server (minimal abstraction over Node `http`) | Vite (dev) / Nitro (prod) | **Elysia** (typed, chainable, WinterCG) |
...
| **Process model (prod)** | 1 process (custom server) or serverless | 1 process (Nitro) or edge | **1 process** (Elysia plugin) or compiled binary |
| **WinterCG compliance** | Partial (uses Node APIs) | Partial (Nitro adapts) | **Native** (Elysia is WinterCG-native) |
| **API routes colocation** | ✅ Route handlers in `app/api` | ✅ API routes via Vinicius | ✅ Elysia plugins colocated with pages |
| **WebSocket / SSE** | Custom implementation | Via Vinicius plugins | **Native Elysia** (`.ws()`, SSE via `ReadableStream`) |
...
| Dimension | Next.js | TanStack Start | Furin |
|---|---|---|---|
| **Self-hosted (Docker)** | ✅ (`next start`) | ✅ (Nitro preset) | ✅ (`bun run server.ts` or compiled binary) |
| **Static export** | ✅ (`output: "export"`) | ✅ | ✅ (`static` target) |
| **Vercel (serverless)** | ✅ (native) | ✅ (via Nitro) | ❌ (planned) |
| **Cloudflare Workers** | ⚠️ (edge runtime) | ✅ (official partner, native guide) | ❌ (planned) |
| **AWS Lambda** | ⚠️ (adapter) | ✅ (via Nitro) | ❌ (planned) |
| **Edge runtime** | ✅ (Edge API Routes) | ⚠️ (Nitro) | ⚠️ (Elysia is WinterCG, but no edge adapter yet) |
...
- You prefer **Nitro** for server primitives.
...
*Last verified: April 22, 2026. Sources: Next.js 15 docs, TanStack Start RC docs, Furin source code.*
