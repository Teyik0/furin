...
- **Loader errors** — any throw from `defineRoute().loader()`
...
In development, loader and render failures use the Furin error overlay instead of the production boundary UI. The overlay includes the original message and cause, source location, route, phase, import chain, and stack trace. It reloads automatically after the development graph publishes a successful revision, or you can select **Retry route**.

These diagnostics are development-only. Production responses continue to use the boundary and digest behavior described above.
