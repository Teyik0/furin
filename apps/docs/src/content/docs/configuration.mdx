...
Path to the server entrypoint that creates the Elysia instance. Required for `bun` and `vercel` target builds. Not needed for `static` builds.
...
Build targets to produce. `"bun"`, `"vercel"`, and `"static"` are fully implemented today. `"node"` and `"cloudflare"` are planned.

| Target | Output | Use case |
|--------|--------|----------|
| `bun` | Server bundle + client assets | Self-hosted Bun runtime |
| `vercel` | Build Output API v3 | Vercel Functions, CDN, SSG, and ISR |
...
**Type:** `string[]`
**Default:** Vercel project default (`iad1` for new projects)

Compute regions written into the generated Vercel Function configuration. Place the Function close to its database or upstream data source:
...
The array must contain at least one unique Vercel region code. Malformed identifiers fail configuration loading.
