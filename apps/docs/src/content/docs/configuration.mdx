...
The notifier may be omitted for local adapters and during development, where Furin polls the adapter cursor. Distributed adapters require an explicit notifier in production so replicas cannot silently generate continuous polling traffic. Notifications are only wake-ups: ordered recovery always reads `adapter.readChanges()`, including after a disconnect or lost notification. Use `postgresSyncNotifier()` for a zero-poll PostgreSQL deployment, or pass `new PollingSyncNotifier(adapter, intervalMs)` as an explicit compatibility fallback.

The adapters are isolated, tree-shakable subpaths of `@teyik0/furin`:

- `@teyik0/furin/sync/postgres` — recommended durable adapter plus native PostgreSQL `LISTEN/NOTIFY`, using `Bun.SQL`.
